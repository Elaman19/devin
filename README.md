...
NestJS backend with Postgres, Redis, OpenSearch and MinIO, running under Docker Compose.
...
Redis, OpenSearch and MinIO are provisioned in Compose and reachable via env vars from day one; the app doesn't have clients wired up for them yet — those get added when a feature needs them.
...
This is the single command that must pass before any work is considered done: it starts a local Postgres via Docker Compose, then runs type checking, linting, format checking and tests with a 100% coverage gate. See `AGENTS.md` for the rule and rationale, and `vitest.config.ts` for the coverage configuration (including the short list of files excluded from coverage, each with a comment explaining why).
